...
class VariantsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create variant" do
    assert_difference('Variant.count') do
      post variants_url, params: { variant: { color: @variant.color, product_id: @variant.product_id, size: @variant.size } }
    end
  ...
  test "should update variant" do
    patch variant_url(@variant), params: { variant: { color: @variant.color, product_id: @variant.product_id, size: @variant.size } }
